...
//		System.out.println(Main.loginMember.getNo()); // 2
	...
	public void searchWriter() throws Exception {
		...
		if(Main.loginMember != null && Main.loginMember.getAdmin_yn().equals("Y")) {
			...
			String sql1 = "SELECT R.FOOD_REVIEW_NO, R.REVIEW_TITLE, R.REVIEW,M.NICK ,TO_CHAR(R.ENROLL_DATE, 'YYYY-MM-DD HH:MI:SS') AS ENROLL_DATE FROM FOOD_REVIEW R JOIN MEMBER M ON R.WRITER_NO = M.NO WHERE M.NICK LIKE ?";
		...
		String sql = "SELECT R.FOOD_REVIEW_NO, R.REVIEW_TITLE, R.REVIEW ,M.NICK ,TO_CHAR(R.ENROLL_DATE, 'YYYY-MM-DD HH:MI:SS') AS ENROLL_DATE FROM FOOD_REVIEW R JOIN MEMBER M ON R.WRITER_NO = M.NO WHERE R.QUIT_YN = 'N'AND M.NICK LIKE ?";
