...
public class NormalProdController {
	...
	public void lookUpName() throws Exception {
		...
		String sql = "SELECT * FROM NORMAL_PRODUCT WHERE NAME = ?";
		...
		for(NormalProdVo vo : volist) {
		   System.out.printf("%-6s | %-15s | %-10s | %-8s | %-20s%n", vo.getNormaProdlNo() , vo.getNormalProdName(),vo.getPrice(), vo.getIsDiscount(), vo.getDescription());
		   }
